...
function getLocaleWeekStartsOn(locale?: string): 0 | 1 | 2 | 3 | 4 | 5 | 6 | undefined {
  try {
    ...
    const firstDay = (intlLocale as Intl.Locale & { weekInfo?: { firstDay?: number } }).weekInfo?.firstDay;
...
export function resolveThemeOption(theme?: ThekDatePickerThemeOption): Partial<ThekDatePickerTheme> {
  if (!theme) return {};
